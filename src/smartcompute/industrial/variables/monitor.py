...
class PLCCommunicationEngine:
    ...
    def __init__(self):
        ...
        self.supported_protocols = {
            ...
            'can_bus':   {'port': None,  'library': 'python-can'},
            'lonworks':  {'port': 1628,  'library': 'socket'},
            'devicenet': {'port': None,  'library': 'python-can'},
        }
    ...
    async def connect_can(self, channel: str = "vcan0", baudrate: int = 500_000):
        """Conectar a CAN bus (SocketCAN o virtual)"""
        return {
            'type': 'can_bus',
            'channel': channel,
            'baudrate': baudrate,
    ...
    async def connect_lonworks(self, host: str, port: int = 1628, domain_id: bytes = None):
        """Conectar a dispositivo LonWorks/IP via UDP"""
        return {
            'type': 'lonworks',
            'socket': await self.create_socket_connection(host, port),
            'domain_id': domain_id,
            'connected': True
        }

    async def connect_devicenet(self, channel: str = "vcan0", mac_id: int = 0):
        """Conectar a red DeviceNet (CAN-based, MAC ID 0-63)"""
        if not 0 <= mac_id <= 63:
            raise ValueError(f"DeviceNet MAC ID must be 0-63, got {mac_id}")
        return {
            'type': 'devicenet',
            'channel': channel,
            'mac_id': mac_id,
    ...
    async def read_can_frame(self, connection: Dict) -> Optional[Dict]:
        """Leer trama CAN del bus"""
        # Simulación: generar trama realista
        can_ids = [0x100, 0x200, 0x300, 0x7DF]
        return {
    ...
    async def read_lon_nv(self, connection: Dict, nv_index: int) -> Optional[float]:
        """Leer Network Variable LonWorks por índice"""
        # Simulación de NV según tipo (temperatura, nivel, etc.)
        return round(random.uniform(0, 100), 2)

    async def read_devicenet_tag(self, connection: Dict, class_id: int, instance: int, attr: int) -> Optional[Any]:
        """Leer atributo DeviceNet (Explicit Messaging: Class/Instance/Attribute)"""
        # Simulación: devolver valor según clase CIP
        if class_id == 0x01:    # Identity Object
            return {'vendor_id': 0x0001, 'device_type': 0x0002, 'status': 0x0004}
        elif class_id == 0x64:  # Vendor-specific analog input
