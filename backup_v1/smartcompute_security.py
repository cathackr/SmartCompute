...
class SmartComputeSecure:
    """SmartCompute con capacidades de seguridad integradas - MEJORADO"""
    ...
    def __init__(self, security_enabled: bool = True):
        ...
        # Security Module (nuevo) - CON CONTROLES
        ...
        if security_enabled:
            ...
            # Registrar cleanup en destructor
            import atexit
            atexit.register(self._emergency_cleanup)
        
        print(f"🚀 SmartCompute Secure iniciado - VERSIÓN SEGURA")
        print(f"⚙️  CPU: {self.cpu_cores} cores")
        print(f"🎮 GPU: {'Disponible' if self.gpu_available else 'No disponible'}")
        print(f"🔒 Security: {'Habilitado con límites' if security_enabled else 'Deshabilitado'}")
        
        if security_enabled:
            print(f"⚠️  Límites de seguridad activos:")
            print(f"   - Máximo 1 hora de monitoreo continuo")
            print(f"   - Máximo 120 iteraciones por sesión")
            print(f"   - Cleanup automático de threads")
    ...
    def stop_security_monitoring(self):
        """Detiene monitoreo de seguridad - MEJORADO"""
        print("🔒 Deteniendo monitoreo de seguridad...")
        self.security_monitoring = False
        
        # Esperar que el thread termine apropiadamente
        if hasattr(self, 'monitor_thread') and self.monitor_thread.is_alive():
            print("🔒 Esperando finalización del thread...")
            self.monitor_thread.join(timeout=5)  # Esperar máximo 5 segundos
            
            if self.monitor_thread.is_alive():
                print("⚠️ Thread no respondió - Forzando detención")
            else:
                print("✅ Thread finalizado correctamente")
        
        print("🔒 Monitoreo de seguridad DETENIDO")
    
    def _security_monitor_loop(self):
        """Loop de monitoreo continuo (thread separado) - CON LÍMITES DE SEGURIDAD"""
        max_runtime = 3600  # LÍMITE: 1 hora máximo
        max_iterations = 120  # LÍMITE: máximo 120 iteraciones (1 hora a 30s)
        start_time = time.time()
        iteration_count = 0
        
        print(f"🔒 Monitor iniciado - Límites: {max_runtime}s runtime, {max_iterations} iteraciones")
        
        while (self.security_monitoring and 
               (time.time() - start_time) < max_runtime and 
            ...
            try:
                ...
                if iteration_count % 10 == 0:
                    print(f"📊 Monitor - Iteración {iteration_count}/{max_iterations}, Tiempo: {elapsed:.1f}s")
                ...
                # Sleep con verificación de interrupción
                for i in range(30):
                ...
                print(f"⚠️ Error en monitoreo (iteración {iteration_count}): {e}")
                # Aumentar delay en caso de errores
                time.sleep(5)
        
        # CLEANUP FORZADO
        self.security_monitoring = False
        elapsed = time.time() - start_time
        print(f"\n🔒 Monitor FINALIZADO - Duración: {elapsed:.1f}s, Iteraciones: {iteration_count}")
        print(f"🔒 Razón: {'Límite de tiempo' if elapsed >= max_runtime else 'Límite de iteraciones' if iteration_count >= max_iterations else 'Detención manual'}")
    ...
    def _emergency_cleanup(self):
        """Cleanup de emergencia ejecutado al salir"""
        if hasattr(self, 'security_monitoring') and self.security_monitoring:
            print("\n🚑 CLEANUP DE EMERGENCIA - Deteniendo monitoreo...")
    ...
    def get_security_status(self) -> Dict:
        """Obtiene estado actual de seguridad - MEJORADO"""
        if not self.security_enabled:
        ...
        # Alertas recientes (corregir cálculo)
        try:
            recent_count = 0
            current_time = time.time()
            for alert in self.anomaly_detector.alerts:
                alert_time = time.mktime(time.strptime(alert['timestamp'][:19], "%Y-%m-%dT%H:%M:%S"))
                if (current_time - alert_time) < 3600:  # Última hora
                    recent_count += 1
            status['recent_alerts'] = recent_count
        except:
            status['recent_alerts'] = 0
